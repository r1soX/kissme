...
func (gs *GameSock) kissesReceive(msg io.Reader) {

	gs.bot.LogINFO("KissesServerPacket", "Try parse server packet")

	//fmt.Println(msg)

	kisses := server.KissesServerPacket{}

	if err := kisses.Parse(msg); err != nil {
		gs.bot.IsError = true
		gs.bot.LogERROR("KissesServerPacket", "Error parse packet")
		return
	}

	gs.bot.Kisses = kisses.Kisses
	gs.bot.LogINFO("KissesServerPacket", fmt.Sprintf("Update kisses: %d", gs.bot.Kisses))
}
